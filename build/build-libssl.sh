...
if [ ! -e openssl-${VERSION}.tar.gz ]; then
    ...
    curl -O ftp://ftp.openssl.org/source/openssl-${VERSION}.tar.gz
else
...
do
  ...
  if [[ "$VERSION" =~ 1.0.0. ]]; then
  		echo "BSD-generic32 --openssldir"
	  ./Configure BSD-generic32 --prefix="${OUTPATH}" --openssldir="${OUTPATH}" > "${LOG}" 2>&1
  else
  	  	echo "iphoneos-cross"
	  ./Configure iphoneos-cross  --prefix="${OUTPATH}" no-asm --openssldir="${OUTPATH}" > "${LOG}" 2>&1
  fi
